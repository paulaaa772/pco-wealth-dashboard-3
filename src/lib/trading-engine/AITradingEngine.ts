...
export class AITradingEngine {
  ...
  public async executeTradeSignal(signal: TradeSignal): Promise<boolean> {
    ...
    if (this.mode === 'live') {
      ...
      console.log('[AI Engine] DEMO MODE: Simulating trade execution with realistic prices.');
      
      // Simulate delay for realism
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // Only modify the price field in the signal object to ensure it matches current market price
      // Don't create unrealistic price jumps - this fixes the issue with BAC going from $39 to $210
      try {
        // Get the current market price to use as the execution price
        const currentPrice = await this.polygonService.getLatestPrice(signal.symbol);
        if (currentPrice) {
          // Update the signal price to use the current market price
          signal.price = currentPrice;
          console.log(`[AI Engine] Updated execution price to current market price: $${currentPrice.toFixed(2)}`);
        }
      } catch (error) {
        console.error(`[AI Engine] Error getting current market price for ${signal.symbol}:`, error);
        // Continue with the existing price if we can't get current market data
      }
  ...
  async analyzeMarket(symbol?: string): Promise<TradeSignal | null> {
    ...
    try {
        ...
        // Error handling for API key, no candles, etc.
        if (candles === null) {
            console.error(`[AI Engine] !!! FAILED TO FETCH REAL CANDLES for ${targetSymbol}. Check API Key/Entitlements.`);
            return null; 
        } else if (candles.length < historyNeeded) {
            console.warn(`[AI Engine] Insufficient candle data for ${targetSymbol} (${candles.length} received). Needed >= ${historyNeeded}.`);
            return null;
        }
        
        console.log(`[AI Engine] Successfully fetched ${candles.length} candles for ${targetSymbol}.`);
        candles.sort((a, b) => a.t - b.t); 
        ...
        // Get current market price - this is more accurate than the last candle
        let currentPrice = latestPrice;
        try {
            const realtimePrice = await this.polygonService.getLatestPrice(targetSymbol);
            if (realtimePrice) {
                console.log(`[AI Engine] Using real-time price: $${realtimePrice} instead of last candle price: $${latestPrice}`);
                currentPrice = realtimePrice;
            }
        } catch (error) {
            console.warn(`[AI Engine] Could not get real-time price, using last candle price: $${latestPrice}`);
        }
